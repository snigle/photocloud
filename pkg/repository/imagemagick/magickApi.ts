...
function ChangeUrl(url :string, fileName:string)
{
    let splitUrl = url.split('/')
    splitUrl[splitUrl.length -1] = fileName
    return splitUrl.join('/')
}
function GetCurrentUrlDifferentFilename(currentUrl : string, fileName : string) {
...
function GenerateStackAndGetPathAtDepth(depth: number){
  try {
...
function GenerateMagickWorkerText(magickUrl: string){
...
const magickWorkerPromises : {[key:number]:any} = {}
