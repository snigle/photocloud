...
export class ImageMagick implements ILocalPhoto {
    constructor() { }

    async compress(file: LocalPhoto): Promise<PhotoRaw> {
        if (!file.content) {
            throw `trying to compress empty photo named ${file.name}`
        }
        const sanitizedName = this.sanitize(file.name)
    ...
    async createThumbnail(file: LocalPhoto): Promise<PhotoRaw> {
        if (!file.content) {
            throw `trying to compress empty photo named ${file.name}`
        }
        const sanitizedName = this.sanitize(file.name)
